...
public:
    ...
    explicit SimpleVector(size_t size) : items_(size), size_(size), capacity_(size) {
        Type it{};
        std::fill(items_.Get(), items_.Get() + size, it);
    }
    ...
    void Resize(size_t new_size) {
        ...
        if (new_size > capacity_) {
            ...
            std::copy(std::make_move_iterator(items_.Get()), std::make_move_iterator(items_.Get() + size_), temp.Get());
            temp.swap(items_);
   ...
   //  метод PushBack перемещением
    ...
    Iterator Insert(ConstIterator pos, const Type& value) {
        ...
        if (size_ = capacity_) {
            if (size_ == 0) {
                ...
                Type it{};
                std::fill(temp.Get(), temp.Get() + capacity_, it);
                items_.swap(temp);
                ++capacity_;
            }
    ...
    Iterator Insert(Iterator pos, Type&& value) {
        ...
        if (capacity_ == 0) {
            ...
            std::move_backward(std::make_move_iterator(items_.Get() + count), std::make_move_iterator(items_.Get() + size_), items_.Get() + size_ + 1);
            std::exchange(items_[count], std::move(value));
        }
        else {
            size_t new_capacity = std::max(size_ + 1, capacity_ * 2);
            ArrayPtr<Type> temp(capacity_);
          
          std::move(std::make_move_iterator(items_.Get()), std::make_move_iterator(items_.Get() + size_), temp.Get());
            std::move_backward(std::make_move_iterator(items_.Get() + count), std::make_move_iterator(items_.Get() + size_), 
                items_.Get() + size_ + 1);
    ...
    Iterator Erase(ConstIterator pos) {
        ...
        size_t count = pos - items_.Get();
        std::move(std::make_move_iterator(items_.Get() + count + 1), std::make_move_iterator(items_.Get() + size_), items_.Get() + count);
        --size_;
    ...
    void Reserve(size_t new_capacity) {
        if (new_capacity > capacity_) {
            ArrayPtr<Type> temp(new_capacity);
            Type it{};
            std::fill(temp.Get(), temp.Get() + new_capacity, it);
            std::copy(std::make_move_iterator(items_.Get()), std::make_move_iterator(items_.Get() + size_), temp.Get());
            items_.swap(temp);
            capacity_ = new_capacity;
        }
